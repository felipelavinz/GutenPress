...
class PostMeta{
	...
	private function actionsManager(){
		...
		add_action('admin_init', array($this, 'buildPostMeta'));
		add_action('admin_notices', array($this, 'adminNotices'));
	}
	public function adminMenu(){
		// initialize variables just before adding the admin page, so they might be filtered
		$this->page_title = __('Custom Post Meta generator', 'gutenpress');
		$this->menu_title = __('Post Meta generator', 'gutenpress');
		$this->capability = apply_filters('gutenpress_build_post_meta_capability', 'edit_plugins');
		$this->menu_slug  = 'gutenpress-build-post_meta';
		$this->function   = apply_filters('gutenpress_build_post_meta_function', array($this, 'adminPage'));
	...
	public function adminNotices(){
		if ( empty($_GET['gp_msg']) )
			return;
		// define messages
		$msgs = apply_filters('gutenpress_build_post_meta_notices', array(
			'post_meta_created' => __('Custom post meta <strong>%1$s</strong> was correctly created.', 'gutenpress')
		));
	...
	private function getElementTypes(){
		$types = array(
			'\GutenPress\Forms\Element\InputText' => __('Text input', 'gutenpress'),
			'\GutenPress\Forms\Element\InputCheckbox' => __('Checkboxes', 'gutenpress'),
			'\GutenPress\Forms\Element\InputRadio' => __('Radio button', 'gutenpress'),
			'\GutenPress\Forms\Element\InputEmail' => __('E-mail', 'gutenpress'),
			'\GutenPress\Forms\Element\Select' => __('Selectable', 'gutenpress'),
			'\GutenPress\Forms\Element\InputUrl' => __('URL', 'gutenpress'),
			'\GutenPress\Forms\Element\InputNumber' => __('Number', 'gutenpress'),
			'\GutenPress\Forms\Element\InputTextMultiple' => __('Multiple text input', 'gutenpress'),
			'\GutenPress\Forms\Element\Textarea' => __('Text area', 'gutenpress'),
			'\GutenPress\Forms\Element\WPEditor' => __('Rich text editor', 'gutenpress'),
			'\GutenPress\Forms\Element\InputFile' => __('File upload', 'gutenpress')
		);
		return apply_filters('gp_build_post_meta_get_element_types', $types);
	}
	public function adminPage(){
		$form = new Forms\Form('gp-build-post_meta', '', array('class' => 'gutenpress-form'));
		$form->addElement( new Element\InputText(
			__('Post Meta Title', 'gutenpress'),
			$form->getName('title'),
			array(
				'required' => 'required',
				'class' => 'regular-text',
				'description' => __('Will be used as metabox title', 'gutenpress')
			)
		) );
		$form->addElement( new Element\InputText(
			__('Post Meta ID', 'gutenpress'),
			$form->getName('id'),
			array(
				'id' => $form->getId('post_meta_id'),
				'required' => 'required',
				'size' => 25,
				'description' => __('A <strong>short and unique</strong> slug to identify this kind of meta data. Used as id for the metabox and prepended to data defined within this class', 'gutenpress')
			)
		) );
		$form->addElement( new Element\FieldsetMultiple(
			$form->getId('meta_data'),
			__('Define meta data', 'gutenpress'),
			array(),
			array(
				new Element\InputText(
					__('Data name', 'gutenpress'),
					'name',
					array(
						'placeholder' => __('A unique slug for this data entry', 'gutenpress'),
						'class' => 'regular-text'
					)
				),
				new Element\Select(
					__('Element type', 'gutenpress'),
					'element',
		...
		// 			),
		// 			array(
		// 				'id' => 'gp-build-post_meta-labels-gender'
		// 			)
		// 		),
		// 		new Element\InputText(
		// 			__('Name', 'gutenpress'),
	...
	public function buildPostMeta(){
		...
		if ( ! isset($_POST['gp-build-post_meta']) )
			return;
		try {
			...
			$postdata = Helpers\Arrays::filterRecursive( $_POST['gp-build-post_meta'] );
